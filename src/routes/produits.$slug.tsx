...
function NotFoundProduct() {
  return (
    <section className="px-5 py-32 text-center">
      <h1 className="font-display text-4xl text-foreground">Outil introuvable</h1>
      <p className="mt-4 text-muted-foreground">Cet outil n'existe pas ou a été retiré.</p>
      <Button asChild className="mt-8 rounded-full">
        <Link to="/boutique">Retour à la banque d'outils</Link>
      </Button>
    </section>
  );
}
    ...
    <>
        ...
        <div className="mx-auto grid max-w-6xl gap-12 md:grid-cols-2 md:items-start">
          ...
          <div>
            ...
            <div className="mt-8 flex items-start gap-3 rounded-2xl border border-border bg-muted/40 p-4 text-xs text-muted-foreground">
              ...
              <p>
                Usage personnel uniquement. Le contenu demeure la propriété intellectuelle d'Evana
                Clinique et ne peut être reproduit ou diffusé sans autorisation.
              </p>
            </div>
          </div>
        </div>
      ...
      <section className="bg-muted/60 px-5 py-16 md:px-8 md:py-24">
        <div className="mx-auto max-w-4xl">
          <h2 className="font-display text-3xl text-foreground md:text-4xl">Description</h2>
      ...
      {related.length > 0 && (
        <section className="px-5 py-20 md:px-8 md:py-28">
          <div className="mx-auto max-w-7xl">
            <h2 className="font-display text-3xl text-foreground md:text-4xl">Autres outils similaires</h2>
      ...
      <CTABand
        title="Une question sur cet outil ?"
        description="Prenez rendez-vous avec une de nos sexologues pour un accompagnement personnalisé."
      />
    </>
  );
}
